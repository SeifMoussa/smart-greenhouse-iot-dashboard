...
def test_export_csv_header_only_when_empty(operator_client) -> None:
    response = operator_client.get("/api/export.csv")
...
def test_export_csv_requires_auth(client) -> None:
    response = client.get("/api/export.csv")
    assert response.status_code == 401


def test_export_csv_includes_rows(operator_client) -> None:
...
def test_export_csv_type_filter(operator_client) -> None:
    operator_client.post(
        "/api/readings",
    ...
    response = operator_client.get("/api/export.csv?type=humidity")
    body = response.text
    assert "humidity" in body
    assert "temperature" not in body


def test_export_csv_range_too_large(operator_client) -> None:
    fr = (datetime.now(UTC) - timedelta(days=60)).isoformat()
    to = datetime.now(UTC).isoformat()
    response = operator_client.get("/api/export.csv", params={"from": fr, "to": to})
